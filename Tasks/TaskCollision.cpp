...
void TaskCollision::setForces() {
    ...
    if (m_handleCollisionByForce) {
        m_collider->handleCollisionByForce(
            ps, m_forceStrength, m_kn_normalFriction, m_kt_tangentialFriction);
    } else {
        m_collider->handleCollisionByVelocity(ps, m_kn_normalFriction,
                                              m_kt_tangentialFriction);
    }
}
...
void TaskCollision::imGui() {
    ImGui::SliderFloat(paramName("m_kn_normalFriction"), &m_kn_normalFriction,
                       0.0f, 1.0f);
    ImGui::SliderFloat(paramName("kt_tangentialFriction"),
                       &m_kt_tangentialFriction, 0.0f, 1.0f);

    ImGui::Separator();

    ImGui::Checkbox(paramName("m_handleCollisionByForce"),
                    &m_handleCollisionByForce);
    if (m_handleCollisionByForce) {
        ImGui::SliderFloat(paramName("m_forceStrength"), &m_forceStrength, 0.0f,
                           1000.0f);
    }
    ...
    if (m_colliderType > 0) // do not animate plane
    {
        ...
        if (m_animateCollider) {
            ImGui::SliderFloat(paramName("m_animateColliderSpeed"),
                               &m_animateColliderSpeed, 0.01f, 0.1f);
        }
    }
}
