...
void TaskLect04_AddForce::setForces() {
    ...
    if (m_multiplyWithMass) {
        for (auto &it : ps.states()) {
            // todo students
            it.force() += it.force() + (f * factor / it.mass());
        }
    } else {
        for (auto &it : ps.states()) {
            // todo students
            it.force() += it.force() + f * factor;
        }
    }

    if (bUseGlobalFriction) {
        for (auto &it : ps.states()) {
            it.vel() *= m_globalFriction;
        }
    } else if (bUseArtificialFriction) {
        for (auto &it : ps.states()) {
            it.force() -= it.vel() * m_artificialFriction;
        }
    }
}
...
void TaskLect04_AddForce::imGui() {
    ...
    ImGui::Checkbox(paramName("Treat as acceleration (i.e., multiplyWithMass)"),
                    &m_multiplyWithMass);
    ImGui::Checkbox(paramName("Divide by number of steps!!!"),
                    &m_divideByNumberOfSteps);
    ImGui::Separator();
    ImGui::SliderFloat(paramName("Global friction"), &m_globalFriction, 0.0f,
                       2.0f);
    ImGui::SliderFloat(paramName("Artificial Friction"), &m_artificialFriction,
                       0.997f, 1.0f);
}
