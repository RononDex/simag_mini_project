...
glm::vec3 TaskFlocking::steerForceCraigReynolds(int particleIdx,
                                                glm::vec3 const &direction) {
    auto &ps = particleSystem(0);
    auto &p = ps.particle(particleIdx);
    ...
    // todo students
    auto desiredVelocity = glm::normalize(direction) * m_maxSpeed;
    steer = desiredVelocity - p.pos();
...
glm::vec3 TaskFlocking::separate(int particleIdx) {
    ...
    for (int i = (int)ps.size() - 1; i >= 0; i--) {
        ...
        if ((d > 0) && (d < m_desiredSeparation)) {
            ...
            // Therefore, normalize the distance and weight by distance (i.e.,
            // divide by d) Acuumulate in steer

            // todo students
            glm::vec3 k = glm::normalize(pos - posOther);
            k /= d;
            steer += k;
...
glm::vec3 TaskFlocking::align(int particleIdx) {
    ...
    if (count > 0) {
        sum /= (float)count;
        if (glm::length(sum) < eps)
            return glm::vec3(0); // avoid division by zero
...
glm::vec3 TaskFlocking::cohesion(int particleIdx) {
    ...
    if (count > 0) {
        sum /= (float)count;
        glm::vec3 desired =
            sum - ps.positions()[particleIdx]; // A vector pointing from the
...
void TaskFlocking::imGui() {
    ...
    ImGui::SliderFloat("Force Intensity Scale##Flocking", &m_forceIntensity,
                       0.1f, 10.0f);
    ...
    ImGui::Separator();
    ImGui::SliderFloat("Separation Force Scale##Flocking", &m_forceSeparation,
                       0.2f, 4.0f);
    ImGui::SliderFloat("Separation Desired Dist##Flocking",
                       &m_desiredSeparation, 0.05f, 0.5f);

    ImGui::SliderFloat("Align Force Scale##Flocking", &m_forceAlign, 0.2f,
                       4.0f);
    ImGui::SliderFloat("Align Neighbor Dist##Flocking", &m_neighborDistAlign,
                       0.05f, 0.5f);

    ImGui::SliderFloat("Cohesion Force Scale##Flocking", &m_forceCohesion, 0.2f,
                       4.0f);
    ImGui::SliderFloat("Cohesion Neighbor Dist##Flocking",
                       &m_neighborDistCohesion, 0.05f, 0.5f);
}
