...
ContextOpenGl::ContextOpenGl(const char *titleWindow)
    : m_sizeFramebuffer(Globals::WINDOW_INIT_WIDTH,
                        Globals::WINDOW_INIT_HEIGHT),
  ...
  m_window = glfwCreateWindow(m_sizeWindow.x, m_sizeWindow.y, titleWindow,
                              m_monitor, nullptr);
  ...
  glfwSetWindowUserPointer(
      m_window, this); // add user pointer to register member as callbacks

  auto funcResize = [](GLFWwindow *w, int x, int y) {
    static_cast<ContextOpenGl *>(glfwGetWindowUserPointer(w))
        ->callbackResize(w, x, y);
  };
  glfwSetFramebufferSizeCallback(m_window, funcResize);

  auto funcKey = [](GLFWwindow *w, int key, int scancode, int action,
                    int mods) {
    static_cast<ContextOpenGl *>(glfwGetWindowUserPointer(w))
        ->callbackKey(w, key, scancode, action, mods);
  };
  glfwSetKeyCallback(m_window, funcKey);

  auto funcChar = [](GLFWwindow *w, unsigned int c) {
    static_cast<ContextOpenGl *>(glfwGetWindowUserPointer(w))
        ->callbackChar(w, c);
  };
  glfwSetCharCallback(m_window, funcChar);

  auto funcMouseButton = [](GLFWwindow *w, int button, int action, int mods) {
    static_cast<ContextOpenGl *>(glfwGetWindowUserPointer(w))
        ->callbackMouseButton(w, button, action, mods);
  };
  glfwSetMouseButtonCallback(m_window, funcMouseButton);

  auto funcCursorPos = [](GLFWwindow *w, double x, double y) {
    static_cast<ContextOpenGl *>(glfwGetWindowUserPointer(w))
        ->callbackCursorPosCallback(w, x, y);
  };
  glfwSetCursorPosCallback(m_window, funcCursorPos);

  auto funcScroll = [](GLFWwindow *w, double xoffset, double yoffset) {
    static_cast<ContextOpenGl *>(glfwGetWindowUserPointer(w))
        ->callbackScroll(w, xoffset, yoffset);
  };
...
double ContextOpenGl::getTime() { return glfwGetTime(); }
...
void ContextOpenGl::setResolution(int widthFramebuffer, int heightFramebuffer,
...
void ContextOpenGl::callbackKey(GLFWwindow *window, int key, int scancode,
                                int action, int mods) {
  ...
  if (action == GLFW_PRESS) {
    if (cameraState.isCameraMoving) {
      cameraState.speedFactorTranslate =
          (mods & GLFW_MOD_SHIFT) ? gEnv->stateGui->cameraSpeedFactorKeyShift
                                  : 1.0f;
      ...
      case 'W':
      case 'w':
        cameraState.moveFwd = 1.0f;
        keyHasBeenHandled = true;
        break;
      case 'S':
      case 's':
        cameraState.moveFwd = -1.0f;
  ...
  if (action == GLFW_RELEASE) {
    ...
    if (cameraState.isCameraMoving) {
      ...
      case 'W':
      case 'w':
        if (cameraState.moveFwd > 0)
          cameraState.moveFwd = 0.0f;
        keyHasBeenHandled = true;
        break;
      case 'S':
      case 's':
      ...
      case 'D':
      case 'd':
        if (cameraState.moveRight > 0)
          cameraState.moveRight = 0.0f;
        keyHasBeenHandled = true;
        break;
      case 'A':
      case 'a':
      ...
      case 'E':
      case 'e':
        if (cameraState.moveUp > 0)
          cameraState.moveUp = 0.0f;
        keyHasBeenHandled = true;
        break;
      case 'Q':
      case 'q':
      ...
      updateCameraState(window); // ensure new camera parameters on key-release,
                                 // to stabilize camera after euler-rotation
    }
  }
...
void ContextOpenGl::callbackMouseButton(GLFWwindow *window, int button,
                                        int action, int mods) {
  // Forward go ImGui
  ...
  if (bLeft) {
    ...
    gEnv->stateGui->mouseButtonLeftRelease =
        bRelease; // pre frame: set to false in prePollEvents()
    gEnv->stateGui->mouseButtonLeftClick =
        bPress; // pre frame: set to false in prePollEvents()
  }
...
void ContextOpenGl::callbackCursorPosCallback(GLFWwindow *window, double xpos,
                                              double ypos) {
  if (gEnv->stateGui->cameraState.isCameraMoving) {
    gEnv->stateGui->cameraState.rotatePosSSprevious =
        gEnv->stateGui->cameraState.rotatePosSSactual;
    gEnv->stateGui->cameraState.rotatePosSSactual =
  ...
  // debug
  // glm::vec2 posWS = gEnv->camera->SSToWorld2d(glm::vec2((float)xpos,
  // (float)ypos)); std::cerr << "debug: " << posWS.x << " / " << posWS.y <<
  // "\n"; glm::vec2 posSS = gEnv->camera->worldToSS2d(posWS); std::cerr <<
  // "DOUBLE CHECK: debug: " << posSS.x << " / " << posSS.y << "        Truth:"
  // << xpos << ":" << ypos << "\n";
...
void ContextOpenGl::callbackScroll(GLFWwindow *window, double xoffset,
                                   double yoffset) {
  ...
  if (gEnv->stateGui->cameraState.isCameraMoving || bCam2d) {
    ...
    const float scaleFactor =
        (yoffset > 0) ? cScaleFactor : (1.0f / cScaleFactor);
